...
class AppOcr {
    ...
    static func extractStockData(from image: UIImage, completion: @escaping ([ExtractedStock]) -> Void) {
        ...
        let request = VNRecognizeTextRequest { request, error in
            ...
            let extractedStocks = parseStockData(from: recognizedText)
    ...
    private static func parseStockData(from text: String) -> [ExtractedStock] {
        ...
        for line in lines {
            ...
            if let stockSymbol = extractStockSymbol(from: trimmedLine) {
                ...
                let stock = createStockFromData(symbol: stockSymbol, numbers: numbers, originalLine: trimmedLine)
        ...
        let uniqueStocks = Dictionary(grouping: extractedStocks, by: { $0.symbol })
            .mapValues { stocks in
                stocks.max { stock1, stock2 in
                    let score1 = (stock1.quantity != nil ? 1 : 0) + (stock1.totalInvestment != nil ? 1 : 0) + (stock1.avgPrice != nil ? 1 : 0)
                    let score2 = (stock2.quantity != nil ? 1 : 0) + (stock2.totalInvestment != nil ? 1 : 0) + (stock2.avgPrice != nil ? 1 : 0)
    ...
    private static func extractStockSymbol(from text: String) -> String? {
        ...
        if let match = regex?.firstMatch(in: text, range: range) {
            ...
            let excludedWords = ["THE", "AND", "FOR", "YOU", "ARE", "NOT", "BUT", "CAN", "GET", "ALL", "NEW", "NOW", "WAY", "USE", "HER", "HIM", "ONE", "TWO", "HOW", "ITS", "OUR", "OUT", "DAY", "MAY", "SAY", "SHE", "TRY", "WHO", "BOY", "DID", "HAS", "LET", "PUT", "TOO", "OLD", "ANY", "APP", "BUY", "TOP", "BAD", "BIG", "YES", "WIN", "END"]
    ...
    private static func extractNumbers(from text: String) -> [Double] {
        ...
        return matches.compactMap { match in
            let matchRange = Range(match.range, in: text)!
            let numberString = String(text[matchRange]).replacingOccurrences(of: ",", with: "")
            return Double(numberString)
        }
    }
    
    private static func createStockFromData(symbol: String, numbers: [Double], originalLine: String) -> ExtractedStock {
        var quantity: Double?
        ...
        if numbers.count >= 2 {
            ...
            if largestNumber > smallestNumber * 10 {
                ...
                if let potentialQuantity = sortedNumbers.first(where: { $0 < 10000 && $0 > 0 }) {
