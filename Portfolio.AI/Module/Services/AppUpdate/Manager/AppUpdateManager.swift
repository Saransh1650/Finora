...
class AppUpdateManager: ObservableObject {
    ...
    func showTestUpdateDialog(isForceUpdate: Bool = false) {
        let testResult = AppUpdateRepo.checkForUpdateWithCustomLogic(
    ...
    private func performUpdateCheck(isManual: Bool) async {
        ...
        let result = await AppUpdateRepo.checkForUpdate()
    ...
    private func shouldShowUpdateDialog(
        ...
        let skippedVersion = userDefaults.string(
            forKey: UserDefaultsKeys.skippedVersion
        )
    ...
    func getLastUpdateCheckDate() -> Date? {
        return userDefaults.object(forKey: UserDefaultsKeys.lastUpdateCheckDate)
            as? Date
    }
}
