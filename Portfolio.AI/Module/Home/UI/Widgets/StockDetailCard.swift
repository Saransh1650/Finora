...
struct StockDetailCard: View {
    let stock: PortfolioAnalysisModel.Stock
            ...
            VStack(spacing: 8) {
                HStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        HStack(spacing: 4) {
                            Text(formatCurrency(stock.currentValue))
                                .font(.caption)
                                .fontWeight(.semibold)
                            Text("(\(String(format: "%.1f", getAllocationPercentage()))%)")
                                .font(.caption)
                                .fontWeight(.semibold)
                        }
                        .foregroundStyle(AppColors.textPrimary)
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("P&L:")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Text("\(stock.pnlPercent >= 0 ? "+" : "")\(String(format: "%.2f", stock.pnlPercent))%")
                            .font(.caption)
                            .fontWeight(.semibold)
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Fair Price Est:")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Text("₹\(String(format: "%.0f", stock.fairPriceEstimate.minPrice)) - ₹\(String(format: "%.0f", stock.fairPriceEstimate.maxPrice))")
                        .font(.caption)
                        .fontWeight(.semibold)
                        .foregroundStyle(AppColors.textPrimary)
                }
...
#Preview {
    StockDetailCard(
        stock: PortfolioAnalysisModel.Stock(
            name: "AAPL",
            invested: 40000,
            currentValue: 42000,
            pnlPercent: 5.0,
            ...
            reason: "High concentration and recent price appreciation suggest a trim. The current price is above the fair value estimate."
        ),
