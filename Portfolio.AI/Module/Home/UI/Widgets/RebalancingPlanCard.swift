...
struct RebalancingPlanCard: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            // Plan items
            VStack(spacing: 16) {
                ForEach(Array(rebalancingPlans.enumerated()), id: \.offset) { index, plan in
    ...
    private func executeRebalancingPlan() {
        // Implementation for executing the rebalancing plan
        ...
        VStack(spacing: 8) {
            ...
            if plan.action.lowercased() == "buy" {
                HStack(spacing: 8) {
                    ...
                    Text("₹\(String(format: "%.0f", plan.fairEntryRange.minPrice)) - ₹\(String(format: "%.0f", plan.fairEntryRange.maxPrice))")
                        .font(.caption2)
                        .fontWeight(.semibold)
                        .foregroundStyle(.green)
