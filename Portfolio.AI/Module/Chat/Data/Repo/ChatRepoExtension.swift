...
extension ChatRepo {
    ...
    static func isMessageTooLongError(_ error: Error) -> Bool {
        ...
        let isMessageTooLong =
            nsError.domain == NSPOSIXErrorDomain && nsError.code == 40
        if isMessageTooLong {
            print(
                "🔴 ChatRepo Debug: Detected Message Too Long Error - Domain: \(nsError.domain), Code: \(nsError.code)"
            )
    ...
    static func shouldRetryError(_ error: Error) -> Bool {
        ...
        if nsError.domain == NSURLErrorDomain
            && retryableURLErrorCodes.contains(nsError.code)
        {
            print(
                "🟡 ChatRepo Debug: Detected retryable URL error - Code: \(nsError.code)"
            )
            return true
        }
    ...
    static func withRetry<T>(
        ...
        while retryCount < maxRetries {
            do {
                ...
                if retryCount > 0 {
                    print(
                        "🟢 ChatRepo Debug: Operation succeeded after \(retryCount) retries"
                    )
                }
                ...
                if shouldRetryError(error) {
                    print(
                        "🟡 ChatRepo Debug: Error is retryable, attempting retry..."
                    )
                    retryCount += 1
                    ...
                    if retryCount < maxRetries {
                        ...
                        print(
                            "🟡 ChatRepo Debug: Waiting \(delay)s before retry \(retryCount + 1)"
                        )
                        ...
                        print(
                            "🔴 ChatRepo Debug: Max retries (\(maxRetries)) reached"
                        )
                    }
                } else {
                    print(
                        "🔴 ChatRepo Debug: Error is not retryable, failing immediately"
                    )
                }
